...
int __cdecl main(int argc, char **argv)
{
    ...
    while (TRUE)
    {
        ...
        if (ip_header != NULL)
        {
            WinDivertHelperFormatIPv4Address(WinDivertHelperNtohl(ip_header->SrcAddr),
                src_str, sizeof(src_str));
            WinDivertHelperFormatIPv4Address(WinDivertHelperNtohl(ip_header->DstAddr),
            ...
            printf("IPv4 [Version=%u HdrLength=%u TOS=%u Length=%u Id=0x%.4X "
                ...
                ip_header->TTL,
                ip_header->Protocol, 
                WinDivertHelperNtohs(ip_header->Checksum), src_str, dst_str);
        }
        if (tcp_header != NULL)
        {
            ...
            if(IsRuleMatch(packet,packet_len,console)){
                SetConsoleTextAttribute(console, FOREGROUND_RED);
                printf("アラート：通信をブロックしました！\n");
                continue;
            }
        ...
        {
            fprintf(stderr, "警告: TCPリセットの送信失敗 (%d)\n",
                GetLastError());
        }
    }
}
